<?php
class SP_Widget_Event_Grid extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_sportspress widget_event_grid widget_sp_event_grid', 'description' => __( 'A grid of events.', 'sportspress' ) );
		parent::__construct('sportspress-event-grid', __( 'Match Grid', 'sportspress' ), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		// Action to hook into
		do_action( 'sportspress_before_widget_template', $args, $instance, 'event-grid' );

		sp_get_template( 'event-grid.php', array( 'id' => $id, 'status' => $status, 'date' => $date, 'date_from' => $date_from, 'date_to' => $date_to, 'date_past' => $date_past, 'date_future' => $date_future, 'date_relative' => $date_relative, 'day' => $day, 'number' => $number, 'order' => $order, 'show_all_events_link' => $show_all_events_link ), '', trailingslashit( SP_SCOREBOARD_DIR ) . 'templates/' );

		// Action to hook into
		do_action( 'sportspress_after_widget_template', $args, $instance, 'event-grid' );

		echo $after_widget;
		do_action( 'sportspress_after_widget', $args, $instance, 'event-grid' );
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		// Filter to hook into
		$instance = apply_filters( 'sportspress_widget_update', $instance, $new_instance, $old_instance, 'event-grid' );

		return $instance;
	}

	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, array( 'title' => '', 'id' => null, 'status' => 'default', 'date' => 'default', 'date_from' => date_i18n( 'Y-m-d' ), 'date_to' => date_i18n( 'Y-m-d' ), 'date_past' => 7, 'date_future' => 7, 'date_relative' => false, 'day' => '', 'number' => 20, 'order' => 'default', 'show_all_events_link' => false ) );
		...
		// Action to hook into
		do_action( 'sportspress_before_widget_template_form', $this, $instance, 'event-grid' );
		?>
		...
		<?php
		// Action to hook into
		do_action( 'sportspress_after_widget_template_form', $this, $instance, 'event-grid' );
	}
}

register_widget( 'SP_Widget_Event_Grid' );
