<?php
/**
 * Event List grouped by Matchday
 *
 * @author 		ThemeBoy
...
$identifier = uniqid( 'eventmatchday_' );
?>
<div class="sp-template sp-template-event-matchday">
	<?php if ( $title ) { ?>
		<h4 class="sp-table-caption"><?php echo $title; ?></h4>
	<?php } ?>
	<div class="sp-table-wrapper">
		<table class="sp-event-matchday sp-event-matchday-format-<?php echo $title_format; ?> sp-data-table<?php if ( $paginated ) { ?> sp-paginated-table<?php } if ( $sortable ) { ?> sp-sortable-table<?php } if ( $responsive ) { echo ' sp-responsive-table '.$identifier; } if ( $scrollable ) { ?> sp-scrollable-table <?php } ?>" data-sp-rows="<?php echo $rows; ?>">
			<!--<thead>
				<tr>
					<?php
