<?php
class SP_Widget_Event_Scoreboard extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_sportspress widget_event_scoreboard widget_sp_event_scoreboard', 'description' => __( 'A list of events.', 'sportspress' ) );
		parent::__construct('sportspress-event-scoreboard', __( 'Scoreboard', 'sportspress' ), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		sp_get_template( 'event-scoreboard.php', array( 'id' => $id, 'status' => $status, 'date' => $date, 'date_from' => $date_from, 'date_to' => $date_to, 'date_past' => $date_past, 'date_future' => $date_future, 'date_relative' => $date_relative, 'day' => $day, 'number' => $number, 'order' => $order, 'show_all_events_link' => $show_all_events_link ), '', trailingslashit( SP_SCOREBOARD_DIR ) . 'templates/' );

		// Action to hook into
		do_action( 'sportspress_after_widget_template', $args, $instance, 'event-scoreboard' );

		echo $after_widget;
		do_action( 'sportspress_after_widget', $args, $instance, 'event-scoreboard' );
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		// Filter to hook into
		$instance = apply_filters( 'sportspress_widget_update', $instance, $new_instance, $old_instance, 'event-scoreboard' );
	...
	function form( $instance ) {
		...
		// Action to hook into
		do_action( 'sportspress_before_widget_template_form', $this, $instance, 'event-scoreboard' );
		?>
		...
		<?php
		// Action to hook into
		do_action( 'sportspress_after_widget_template_form', $this, $instance, 'event-scoreboard' );
	}
}

register_widget( 'SP_Widget_Event_Scoreboard' );
